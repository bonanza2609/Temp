...
def db_connect(db_all,nodb,verbose_level):
  ...
  try:
        if(nodb):
            ...
            db = InfluxDBClient(host=db_Host, port=db_Port, username=db_User, password=db_Password, database=db_Database)
...
def write_database(read_items, sensordata, sensor_dict, db_all, verbose_level):
  ...
  try:
    ...
    if verbose_level>1: print("read_items: " , read_items)                        # sensor_dict = dictionary -> sensor : location
...
def read_records(db_lines,db_all, db_fields_all, verbose_level, db_fields):
  ...
  if verbose_level>1: print("query_count:", query_count)
  ...
  if verbose_level>1: print("stmt:", stmt)
...
def clean_records(db_fields,db_all, db_fields_all, verbose_level):
    ...
    date_time_str = raw_input('Enter Time: ')
    ...
    var = raw_input('Enter yes: ')
    if var == 'yes':
        var = raw_input('Confirm drop Database Entries Enter yes: ')
        if var == 'yes':
